...
class EKSManagedNode:
  ...
  def create_eks_managed_node_cluster(self, config_file):
    ...
    if (
      ...
      self.execute_terraform_command('get', './aws/eks/eks_managed_node/') and
      self.execute_terraform_command('init', './aws/eks/eks_managed_node/') and
      self.execute_terraform_command('apply', './aws/eks/eks_managed_node/')
    ):
      return True
    else:
      return False
